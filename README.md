...
| 機能 | 詳細 |
| --- | --- |
| サーバー状況メッセージ | 指定チャンネルの単一メッセージをEmbedで管理し、状態・オンライン人数・プレイヤー名・補足情報を表示します。メッセージIDは`data/status_message.json`に保存され、Bot再起動後も同じ投稿を更新します。 |
...
- メッセージ本文にも絵文字付きのサマリーを表示し、モバイル環境でも簡潔に状態を確認できます。
