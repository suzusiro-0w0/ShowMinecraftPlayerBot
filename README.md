...
| サーバー起動 | `!start_server` コマンドで外部スクリプトを起動します。サーバーが停止中(`stopped`)でなければエラーを表示し、状況メッセージに実行結果を残します。 |
| サーバー停止 | `!stop_server` コマンドでRCON経由の`stop`指示を送ります。プレイヤーがオンラインの場合はボタン式の確認ダイアログを提示し、キャンセル時は操作を実行しません。 |
| サーバー再起動 | `!restart_server` コマンドで停止→起動を連続実行するか、再起動コマンドを呼び出します。起動中(`running`)以外ではエラーを返します。 |
| 操作通知 | サーバー制御のテキストコマンドを誰が実行したかを状況チャンネルへ一時的に投稿し、一定時間後に自動削除します。 |
| メッセージ整理 | コマンドの応答は対象チャンネルへ返信する方式で返され、状況チャンネルには状況メッセージが常に残ります。 |
...
- Embedはタイトルにオンライン人数を表示し、本文にはプレイヤー名のみを列挙します。参加者がいない場合は「現在参加者はいません」と表示されます。
